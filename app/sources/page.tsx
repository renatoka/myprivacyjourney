    ...
    <div>
      <div className="space-y-3">
        {SOURCES.map((source, index) => (
          <div
            key={index}
            className="flex items-center rounded-lg border border-gray-200 p-3 transition-all duration-300 hover:border-blue-300 hover:bg-blue-50"
          >
            <span className="mr-3 min-w-[30px] text-sm text-gray-500">{index + 1}.</span>
            <a
              href={source}
              target="_blank"
              rel="noopener noreferrer"
              className="flex flex-1 items-center gap-2 text-sm text-blue-600 hover:text-blue-800"
            >
      ...
      <div className="mt-8 text-center">
        <p className="text-xs text-gray-500">Total sources: {SOURCES.length}</p>
        <p className="text-xs text-gray-500">Last updated: September 2025</p>
      </div>
    </div>
  );
}
